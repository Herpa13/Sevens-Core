...
export const CompetitorBrandListView: React.FC<CompetitorBrandListViewProps> = ({ brands, products, onSelectItem, onAddNew }) => {
    ...
    return (
        <div className="bg-slate-800 rounded-lg">
             <div className="p-4 sm:p-6 border-b border-slate-700 flex justify-between items-center">
                ...
                <button
                    onClick={() => onAddNew('competitorBrands')}
            ...
            <div className="space-y-2 p-2">
                {brands.map(brand => {
                    ...
                    const isOpen = openBrands[brand.id as number] || false;
                    return (
                        <div key={brand.id} className="border border-slate-700 rounded-lg bg-slate-800/50 transition-shadow hover:shadow-lg">
                            <button
                                onClick={() => toggleBrand(brand.id as number)}
                                className="w-full flex items-center p-3 text-left focus:outline-none"
                            >
                                <div className="flex-shrink-0 w-10 h-10 bg-slate-700 rounded-full flex items-center justify-center mr-4">
                                    {brand.logoUrl ? (
                                        ...
                                        <Icon name="copyright" className="text-xl text-slate-400" />
                                    )}
                                </div>
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); onSelectItem('competitorBrands', brand)}} className="text-cyan-400 hover:text-cyan-300 p-1 rounded-full"><Icon name="edit"/></button>
                                    <Icon name={isOpen ? 'chevron-down' : 'chevron-right'} className="transition-transform text-slate-500" />
                                </div>
                            </button>
                            {isOpen && (
                                    ...
                                    {associatedProducts.length > 0 ? (
                                        <ul className="space-y-2">
                                            {associatedProducts.map(product => (
                                                <li key={product.id} onClick={() => onSelectItem('competitorProducts', product)} className="flex items-center p-2 rounded-md hover:bg-slate-700 cursor-pointer">
                                                    <Icon name="box" className="mr-3 text-slate-500"/>
                                        ...
                                        <p className="text-sm text-slate-500 text-center py-4">No hay productos asociados a esta marca.</p>
                                    )}
                                </div>
                            )}
                        </div>
