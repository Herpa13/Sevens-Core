...
export const VideoCompositionTemplateDetailView: React.FC<VideoCompositionTemplateDetailViewProps> = ({ initialData, onSave, onDelete, onCancel, appData, setIsDirty, setSaveHandler }) => {
  ...
  const onDragStartCanvas = (e: React.DragEvent<HTMLDivElement>, templateId: number) => {
  ...
  const onDropOnItem = (e: React.DragEvent<HTMLDivElement>, targetId: number) => {
    e.preventDefault();
    e.stopPropagation();
    const dropData = JSON.parse(e.dataTransfer.getData("application/json"));
  ...
  const removeSequence = (idToRemove: number) => {
    setData(prev => ({
        ...prev,
                    ...
                    <div className="space-y-2">
                        {compositionSequences.map((template, index) => (
                             <DroppedSequenceTemplate 
                                ...
                                template={template} 
                                onRemove={() => removeSequence(template.id as number)}
                                onDragStart={(e) => onDragStartCanvas(e, template.id as number)}
                                onDrop={(e) => onDropOnItem(e, template.id as number)}
                             />
                        ))}
                    </div>
                )}
