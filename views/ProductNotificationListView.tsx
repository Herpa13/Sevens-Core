...
type Grouping = 'flat' | 'product' | 'country';
...
export const ProductNotificationListView: React.FC<ProductNotificationListViewProps> = ({ notifications, products, countries, onSelectItem, onAddNew }) => {
  ...
  const groupedData = useMemo(() => {
    ...
    const groups: Record<string, typeof enrichedNotifications> = {};
  ...
  const renderTableRows = (items: typeof enrichedNotifications) => {
    return items.map(item => {
        const config = statusConfig[item.status];
        return (
            <tr key={item.id} className="hover:bg-slate-700/50 cursor-pointer" onClick={() => onSelectItem('productNotifications', item)}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-200">{item.productName}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-400">
                    {item.country ? (
                        <span className="inline-flex items-center bg-slate-700 text-slate-200 text-sm font-medium px-2.5 py-1 rounded-full">
  ...
  return (
    <div className="bg-slate-800 rounded-lg overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-700 flex justify-between items-center">
            ...
            <button
                onClick={() => onAddNew('productNotifications')}
