...
exports.KnowledgeBaseView = void 0;
...
const EntryCard = ({ entry, onClick }) => {
    ...
    return (<div onClick={onClick} className={`bg-slate-800/60 p-4 rounded-lg border border-slate-700 hover:border-cyan-500 cursor-pointer flex flex-col justify-between h-full ${entry.status === 'Archivado' ? 'opacity-60' : ''}`}>
            <div>
                <div className="flex justify-between items-start">
                    ...
                    <Icon_1.Icon name={typeConf.icon} className={`text-2xl ${typeConf.color}`}/>
                </div>
                <div className="mt-2">
...
const KnowledgeBaseView = ({ appData, onSelectItem, onAddNew }) => {
    ...
    return (<div className="bg-slate-800 rounded-lg">
            <div className="p-4 sm:p-6 border-b border-slate-700 flex justify-between items-center">
                ...
                <button onClick={() => onAddNew('knowledgeBaseEntries')} className="flex items-center px-4 py-2 bg-cyan-500 text-slate-900 font-semibold rounded-md hover:bg-cyan-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-cyan-500">
            ...
            <div className="p-4 sm:p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <TextInput_1.TextInput placeholder="Buscar por título, descripción o etiqueta..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="md:col-span-2"/>
                    <Select_1.Select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}>
                        <option value="">Todas las categorías</option>
                        {CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                    </Select_1.Select>
                     <Select_1.Select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}>
                    ...
                    <div className="flex items-center md:col-start-2 lg:col-start-4">
                        <input id="showArchived" type="checkbox" checked={showArchived} onChange={e => setShowArchived(e.target.checked)} className="h-4 w-4 text-cyan-500 border-slate-600 rounded bg-slate-700 focus:ring-cyan-500 focus:ring-offset-slate-900"/>
                        <label htmlFor="showArchived" className="ml-2 text-sm text-slate-300">
                            Mostrar Archivados
                        </label>
                    </div>
                </div>
                 {filteredEntries.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredEntries.map(entry => (<EntryCard key={entry.id} entry={entry} onClick={() => onSelectItem('knowledgeBaseEntries', entry)}/>))}
...
exports.KnowledgeBaseView = KnowledgeBaseView;
