...
exports.AmazonFlashDealDashboard = void 0;
...
const KPICard = ({ title, value, icon, iconColor }) => (<div className="bg-slate-800/60 p-4 rounded-lg">
        <div className="flex items-center">
            <div className={`p-2 rounded-full mr-3 ${iconColor}/20`}>
                <Icon_1.Icon name={icon} className={`text-xl ${iconColor}`}/>
            </div>
            <div>
...
const GanttTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        const data = payload[0].payload;
        if (!data.productName)
            return null;
        return (<div className="bg-slate-800/90 backdrop-blur-sm p-3 border border-slate-600 rounded-md shadow-lg text-xs">
...
const FlashDealTimeline = ({ deals }) => {
    const [timeRange, setTimeRange] = (0, react_1.useState)('month');
    const { chartData, domain } = (0, react_1.useMemo)(() => {
        const now = new Date();
    ...
    return (<div className="h-[600px] w-full p-4">
            ...
            {chartData.length === 0 ? (<p className="text-center text-slate-500 p-8">No hay ofertas programadas en el rango de tiempo seleccionado.</p>) : (<recharts_1.ResponsiveContainer>
                    <recharts_1.BarChart layout="vertical" data={chartData} margin={{ top: 5, right: 30, left: 150, bottom: 5 }} barCategoryGap={20}>
                        <recharts_1.CartesianGrid strokeDasharray="3 3" stroke="#374151"/>
                        <recharts_1.XAxis type="number" domain={domain} tickFormatter={(time) => new Date(time).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })} stroke="#9ca3af" tick={{ fontSize: 10 }}/>
                        <recharts_1.YAxis type="category" dataKey="task" stroke="#9ca3af" width={200} tick={{ fontSize: 12 }}/>
                        <recharts_1.Tooltip content={<GanttTooltip />}/>
                        <recharts_1.Bar dataKey="start" stackId="a" fill="transparent"/>
                        <recharts_1.Bar dataKey="duration" stackId="a">
                            {chartData.map((entry, index) => (<recharts_1.Cell key={`cell-${index}`} fill={getColor(entry.status)}/>))}
                        </recharts_1.Bar>
                    </recharts_1.BarChart>
                </recharts_1.ResponsiveContainer>)}
        </div>);
};
const AmazonFlashDealDashboard = ({ appData, onSelectItem, onAddNew }) => {
    const [filterStatus, setFilterStatus] = (0, react_1.useState)('');
    const enrichedDeals = (0, react_1.useMemo)(() => {
    ...
    return (<div className="bg-slate-800 rounded-lg">
            <div className="p-4 sm:p-6 border-b border-slate-700 flex justify-between items-center">
                ...
                <button onClick={() => onAddNew('amazonFlashDeals')} className="flex items-center px-4 py-2 bg-cyan-500 text-slate-900 font-semibold rounded-md hover:bg-cyan-600">
                    <Icon_1.Icon name="plus" className="mr-2"/> Nueva Oferta Flash
                </button>
            </div>
            ...
            <div className="p-4 sm:p-6">
                ...
                <CollapsibleSection_1.CollapsibleSection title="Timeline de Ofertas (Gantt)" defaultOpen>
                ...
                <div className="overflow-x-auto border border-slate-700 rounded-lg mt-6">
                    <table className="min-w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-700">
                            {filteredDeals.map(deal => (<tr key={deal.id} className="hover:bg-slate-700/50">
                                    ...
                                    <td className="p-3">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusConfig[deal.status].color}`}>
                                            <Icon_1.Icon name={statusConfig[deal.status].icon} className="mr-1.5"/>
                                    ...
                                    <td className="p-3 text-center space-x-2">
                                        <button onClick={() => onSelectItem('amazonFlashDeals', deal)} title="Editar" className="p-2 text-slate-400 hover:text-cyan-400"><Icon_1.Icon name="edit"/></button>
                                        <button title="Duplicar" className="p-2 text-slate-400 hover:text-indigo-400"><Icon_1.Icon name="copy"/></button>
                                        {(deal.status === 'Programada' || deal.status === 'Borrador') && <button title="Cancelar" className="p-2 text-slate-400 hover:text-red-400"><Icon_1.Icon name="ban"/></button>}
                                    </td>
                                </tr>))}
                        </tbody>
                    </table>
...
exports.AmazonFlashDealDashboard = AmazonFlashDealDashboard;
