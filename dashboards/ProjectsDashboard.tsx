...
import type { AppData, Proyecto, Entity, EntityType } from '../types';
...
const ProjectCard: React.FC<{
    project: Proyecto;
    tasks: AppData['tasks'];
    owner: AppData['users'][0] | undefined;
    onClick: () => void;
}> = ({ project, tasks, owner, onClick }) => {
    ...
    return (
        <div onClick={onClick} className="bg-slate-800/60 p-4 rounded-lg border border-slate-700 hover:border-cyan-500 cursor-pointer flex flex-col justify-between h-full">
            <div>
                ...
                <p className="text-xs text-slate-400">Iniciado por: {owner?.name || 'Desconocido'}</p>
                 <span className={`mt-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                    project.status === 'Activo' ? 'bg-blue-500/20 text-blue-300' : 'bg-green-500/20 text-green-300'
                 }`}>
                    {project.status}
                </span>
            </div>
            <div className="mt-4">
                ...
                <div className="flex justify-between items-center text-xs text-slate-400 mt-2">
                    ...
                    {overdueTasks > 0 && (
                        <span className="text-red-400 font-semibold">{overdueTasks} Atrasadas</span>
                    )}
                </div>
            </div>
        </div>
...
export const ProjectsDashboard: React.FC<ProjectsDashboardProps> = ({ appData, onSelectItem }) => {
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {proyectos.map(project => (
                    <ProjectCard
                        ...
                        onClick={() => onSelectItem('proyectos', project)}
                    />
                ))}
             </div>
        ) : (
