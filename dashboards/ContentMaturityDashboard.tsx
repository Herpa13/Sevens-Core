...
// FIX: Correctly import PublicationPlanning type
import { AppData, Product, Platform, AmazonContentPlanningItem, EntityType, Entity, PublicationPlanning } from '../types';
...
const statusIconConfig: Record<keyof PublicationPlanning['planningStatus'], { name: string, icon: string }> = {
...
const StatusIcon: React.FC<{
    // FIX: Update type to correctly reference PublicationPlanning
    item: AmazonContentPlanningItem | undefined;
    type: keyof PublicationPlanning['planningStatus'];
}> = ({ item, type }) => {
...
export const ContentMaturityDashboard: React.FC<ContentMaturityDashboardProps> = ({ appData, onSelectItem }) => {
            ...
            <thead className="bg-slate-700/50 sticky top-0">
              <tr>
                ...
                {amazonCountries.map(country => (
                  <th key={country.id} className="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase tracking-wider">
                    Amazon {country.iso}
                  </th>
                ))}
              </tr>
                ...
                <tr key={product.id}>
                  ...
                  {amazonCountries.map(country => {
                    ...
                    return (
                      <td 
                        key={country.id} 
                        className="px-2 py-2 whitespace-nowrap text-center text-sm border-l border-slate-700/50 hover:bg-slate-700/50 cursor-pointer"
                        onClick={() => onSelectItem('products', product)}
                      >
                         {planningStatus ? (
