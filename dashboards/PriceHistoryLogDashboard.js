...
exports.PriceHistoryLogDashboard = void 0;
...
const FilterPillsSelector = ({ options, selectedIds, onSelectionChange, placeholder }) => {
    ...
    return (<div className="space-y-2 p-3 bg-slate-900/50 rounded-lg border border-slate-700">
            <div className="flex justify-between items-center gap-2">
                 <div className="relative flex-grow">
                    <TextInput_1.TextInput type="text" placeholder={placeholder} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8 text-sm !py-1 w-full"/>
            ...
            return (<button key={option.id} onClick={() => handleToggle(option.id)} className={`px-3 py-1 text-xs font-semibold rounded-full whitespace-nowrap transition-colors ${isSelected ? 'bg-cyan-500 text-slate-900' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>
                            {option.name}
                        </button>);
        })}
            </div>
...
const AnalysisPanel = ({ appData }) => {
    const [selectedPlatformIds, setSelectedPlatformIds] = (0, react_1.useState)([]);
    const [selectedProductIds, setSelectedProductIds] = (0, react_1.useState)([]);
    const filteredLogs = (0, react_1.useMemo)(() => {
        if (!selectedPlatformIds?.length && !selectedProductIds?.length)
            return [];
    ...
    const groupedByDate = (0, react_1.useMemo)(() => {
        const groups = {};
    ...
    return (<CollapsibleSection_1.CollapsibleSection title="Panel de Análisis de Cambios de Precio" defaultOpen>
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                 <FilterPillsSelector options={appData.platforms.map(p => ({ id: p.id, name: p.name }))} selectedIds={selectedPlatformIds} onSelectionChange={setSelectedPlatformIds} placeholder="Buscar plataformas..."/>
                 <FilterPillsSelector options={appData.products.map(p => ({ id: p.id, name: p.name }))} selectedIds={selectedProductIds} onSelectionChange={setSelectedProductIds} placeholder="Buscar productos..."/>
            </div>
            <div className="p-4 max-h-96 overflow-y-auto">
                {Object.entries(groupedByDate).map(([date, logs]) => (<div key={date} className="mb-4">
                ...
                return (<div key={log.id} className="grid grid-cols-4 gap-2 p-2 bg-slate-700/50 rounded-md text-xs">
                                    ...
                                    <div className="text-cyan-400 truncate" title={log.source.name}>
                                        <Icon_1.Icon name="gavel" className="mr-1"/>{log.source.name}
                                    </div>
                                </div>);
            })}
                        </div>
                    </div>))}
                 {filteredLogs.length === 0 && (<p className="text-center text-slate-500 py-8">Selecciona plataformas o productos para ver su historial de precios.</p>)}
            </div>
         </CollapsibleSection_1.CollapsibleSection>);
};
const FullLogPanel = ({ appData }) => {
    const [filters, setFilters] = (0, react_1.useState)({});
    const sortedLogs = (0, react_1.useMemo)(() => [...appData.priceHistoryLogs].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()), [appData.priceHistoryLogs]);
    const filteredLogs = (0, react_1.useMemo)(() => {
        return sortedLogs.filter(log => {
            const product = appData.products.find(p => p.id === log.productId);
            const platform = appData.platforms.find(p => p.id === log.platformId);
            if (filters.product && !product?.name.toLowerCase().includes(filters.product.toLowerCase()))
                return false;
            if (filters.platform && !platform?.name.toLowerCase().includes(filters.platform.toLowerCase()))
                return false;
            if (filters.user && !log.userName.toLowerCase().includes(filters.user.toLowerCase()))
                return false;
            if (filters.date && !new Date(log.timestamp).toLocaleDateString().includes(filters.date))
                return false;
            return true;
        });
    ...
    return (<CollapsibleSection_1.CollapsibleSection title="Registro Detallado de Cambios" defaultOpen>
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-700">
                    <thead className="bg-slate-700/50">
                        <tr>
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400 uppercase"><TextInput_1.TextInput placeholder="Filtrar Fecha..." className="!text-xs" onChange={e => handleFilterChange('date', e.target.value)}/></th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400 uppercase"><TextInput_1.TextInput placeholder="Filtrar Usuario..." className="!text-xs" onChange={e => handleFilterChange('user', e.target.value)}/></th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400 uppercase"><TextInput_1.TextInput placeholder="Filtrar Producto..." className="!text-xs" onChange={e => handleFilterChange('product', e.target.value)}/></th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400 uppercase"><TextInput_1.TextInput placeholder="Filtrar Plataforma..." className="!text-xs" onChange={e => handleFilterChange('platform', e.target.value)}/></th>
            ...
            return (<tr key={log.id}>
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-cyan-400" title={log.source.name}>
                                <Icon_1.Icon name={log.source.type === 'rule' ? 'gavel' : 'tag'} className="mr-2"/>
                                {log.source.name}
                            </td>
                            </tr>);
        })}
                 ...
                 {filteredLogs.length === 0 && (<div className="text-center p-12 text-slate-500">
...
const PriceHistoryLogDashboard = ({ appData }) => {
...
exports.PriceHistoryLogDashboard = PriceHistoryLogDashboard;
