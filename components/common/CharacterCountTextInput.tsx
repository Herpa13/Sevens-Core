...
interface CharacterCountTextInputProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  maxLength: number;
  criticalLength?: number; // e.g., for mobile view
  highlightedKeyword?: string;
  allKeywordsToHighlight?: Array<{ name: string; total: number }>;
}

const renderHighlightedText = (
    text: string, 
    allKeywords: Array<{ name: string; total: number }>, 
...
export const CharacterCountTextInput: React.FC<CharacterCountTextInputProps> = ({ maxLength, criticalLength = 80, value, highlightedKeyword, allKeywordsToHighlight = [], ...props }) => {
  ...
  return (
    <div>
        <div className="relative">
            ...
            <div 
                className="block w-full px-3 py-2 border border-slate-700 rounded-md shadow-sm sm:text-sm bg-slate-900 text-transparent pointer-events-none whitespace-pre-wrap min-h-[42px]"
                aria-hidden="true"
            >
            ...
            <textarea
                ...
                maxLength={maxLength}
                {...props}
                className="absolute inset-0 block w-full px-3 py-2 border border-transparent rounded-md shadow-sm placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm bg-transparent text-transparent caret-cyan-400 resize-none"
                rows={props.rows || 1}
            />
        </div>
