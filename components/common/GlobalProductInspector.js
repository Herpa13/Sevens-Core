...
exports.GlobalProductInspector = void 0;
const react_1 = __importStar(require("react"));
const Icon_1 = require("./Icon");
const TextInput_1 = require("./TextInput");
const CollapsibleSection_1 = require("./CollapsibleSection");
const FieldDisplay = ({ label, value, isHtml = false }) => {
    const displayValue = (0, react_1.useMemo)(() => {
        if (value === null || value === undefined)
            return <i className="text-slate-500">N/A</i>;
        if (Array.isArray(value))
            return value.join(', ');
        if (typeof value === 'boolean')
            return value ? <span className="font-bold text-green-400">Sí</span> : <span className="font-bold text-red-400">No</span>;
    ...
    return (<div className="grid grid-cols-3 gap-2 text-xs items-start py-1.5 border-b border-slate-700/50 relative group">
            <strong className="text-slate-400 col-span-1">{label}</strong>
            {isHtml ? (<div className="text-slate-300 col-span-2 break-words" dangerouslySetInnerHTML={{ __html: String(displayValue) }}></div>) : (<span className="text-slate-300 col-span-2 break-words">{displayValue}</span>)}
...
const GlobalProductInspector = ({ appData, isOpen, onClose }) => {
    ...
    const filteredProducts = (0, react_1.useMemo)(() => {
        ...
        if (!lowerSearch)
            return appData.products.slice(0, 15);
        return appData.products.filter(p => p.name.toLowerCase().includes(lowerSearch) ||
    ...
    const renderProductDetails = () => {
        if (!selectedProduct)
            return null;
        const activeAmazonContentByCountry = {};
        ...
        return (<div className="space-y-2">
            <CollapsibleSection_1.CollapsibleSection title="Información General">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Media y Enlaces">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Marketing Estratégico">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Pesos y Medidas">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Uso y Dosificación">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Etiquetado y Regulatorio">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Envase y Caducidad">
            ...
            <CollapsibleSection_1.CollapsibleSection title="Relaciones">
             ...
             <CollapsibleSection_1.CollapsibleSection title="Composición">
                {selectedProduct.composition?.map((item, index) => {
                const ingredient = appData.ingredients.find(i => i.id === item.ingredientId);
                return (<FieldDisplay key={index} label={ingredient?.latinName || `ID: ${item.ingredientId}`} value={`${item.quantity} ${ingredient?.measureUnit}`}/>);
            })}
            </CollapsibleSection_1.CollapsibleSection>

            {selectedProduct.shopifyContent && (<CollapsibleSection_1.CollapsibleSection title="Contenido Shopify">
                ...
                {selectedProduct.shopifyContent.faqs?.map((faq, i) => (<react_1.default.Fragment key={i}>
            ...
            {selectedProduct.layer2Content && (<CollapsibleSection_1.CollapsibleSection title="Contenido Layer2">
                ...
                {selectedProduct.layer2Content.title.translations.map(t => (<FieldDisplay key={t.lang} label={`Título (${t.lang})`} value={t.finalVersion}/>))}
                {selectedProduct.layer2Content.description.translations.map(t => (<FieldDisplay key={t.lang} label={`Descripción (${t.lang})`} value={t.finalVersion}/>))}
            </CollapsibleSection_1.CollapsibleSection>)}
            ...
            {Object.entries(activeAmazonContentByCountry).map(([iso, content]) => {
                ...
                if (!version)
                    return null;
                return (<CollapsibleSection_1.CollapsibleSection key={iso} title={`Contenido Amazon (${iso})`}>
                        ...
                        {version.content.bulletPoints.map((bp, i) => (<FieldDisplay key={i} label={`Bullet ${i + 1}`} value={bp.text}/>))}
             ...
             <CollapsibleSection_1.CollapsibleSection title="Atributos Amazon">
                {selectedProduct.amazonAttributes && Object.keys(selectedProduct.amazonAttributes).length > 0 ? (Object.entries(selectedProduct.amazonAttributes).map(([key, value]) => (<FieldDisplay key={key} label={key} value={value}/>))) : (<div className="text-xs text-slate-500 p-2">No hay atributos de Amazon definidos para este producto.</div>)}
             </CollapsibleSection_1.CollapsibleSection>
        </div>);
    };
    if (!isOpen)
        return null;
    return (<div className="fixed top-20 right-8 bg-slate-800/95 backdrop-blur-sm border border-slate-700 rounded-lg shadow-2xl w-full max-w-lg z-40 max-h-[80vh] flex flex-col">
      <div className="p-3 border-b border-slate-700 flex justify-between items-center flex-shrink-0 cursor-move">
        <h3 className="text-base font-semibold text-slate-200 flex items-center">
            <Icon_1.Icon name="box-open" className="mr-2 text-indigo-400"/>
        ...
        {selectedProduct ? (<>
                <div className="p-3 flex justify-between items-center flex-shrink-0 border-b border-slate-700">
                    <button onClick={handleBackToSearch} className="text-sm text-cyan-400 hover:underline">
                        <Icon_1.Icon name="arrow-left" className="mr-2"/> Volver a la Búsqueda
                    </button>
                ...
                <div className="p-3 flex-shrink-0">
                    <TextInput_1.TextInput placeholder="Buscar producto por Nombre o SKU..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
                <div className="p-3 space-y-2 overflow-y-auto">
                    {filteredProducts.map(product => (<div key={product.id} onClick={() => handleSelectProduct(product)} className="p-2 bg-slate-700/50 rounded-md hover:bg-slate-700 cursor-pointer">
...
exports.GlobalProductInspector = GlobalProductInspector;
