...
exports.GlobalTranslationPanel = void 0;
const react_1 = __importStar(require("react"));
const Icon_1 = require("./Icon");
const TextInput_1 = require("./TextInput");
const GlobalTranslationPanel = ({ appData, isOpen, onClose }) => {
    const [searchTerm, setSearchTerm] = (0, react_1.useState)('');
    const [expandedTermId, setExpandedTermId] = (0, react_1.useState)(null);
      ...
      <div className="p-3 space-y-2 overflow-y-auto">
        {filteredTerms.map(term => {
            ...
            return (<div key={term.id} className="bg-slate-700/50 p-2 rounded-md">
                    <button onClick={() => setExpandedTermId(isExpanded ? null : term.id)} className="font-bold text-sm text-slate-200 text-left w-full flex justify-between items-center">
                        <span>{spanishTranslation}</span>
                        <Icon_1.Icon name={isExpanded ? 'chevron-down' : 'chevron-right'} className="text-slate-400 transition-transform"/>
                    </button>
                    {isExpanded && (<div className="mt-2 pt-2 border-t border-slate-600 grid grid-cols-2 gap-x-2 gap-y-1">
...
exports.GlobalTranslationPanel = GlobalTranslationPanel;
