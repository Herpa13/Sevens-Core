...
export const ApplySchemaModal: React.FC<ApplySchemaModalProps> = ({ schemas, onApply, onClose, entityName }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl border border-slate-700 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-3 overflow-y-auto">
          {schemas.map(schema => (
            <div key={schema.id} className="bg-slate-700/50 p-3 rounded-md border border-slate-600 flex justify-between items-center">
                <div>
                ...
                <button
                    ...
                    className="px-4 py-2 bg-cyan-500 text-slate-900 font-semibold rounded-md hover:bg-cyan-600 flex-shrink-0"
                >
                    Aplicar
                </button>
            </div>
