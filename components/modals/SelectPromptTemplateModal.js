...
exports.SelectPromptTemplateModal = void 0;
const react_1 = __importDefault(require("react"));
const Icon_1 = require("../common/Icon");
const SelectPromptTemplateModal = ({ appData, category, onClose, onSelect }) => {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl border border-slate-700 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-3 overflow-y-auto">
          {relevantTemplates.length > 0 ? (relevantTemplates.map(template => (<button key={template.id} onClick={() => onSelect(template.id)} className="w-full text-left bg-slate-700/50 p-3 rounded-md border border-slate-600 hover:border-cyan-500 transition-colors">
                <h4 className="font-bold text-slate-200">{template.name}</h4>
                <p className="text-sm text-slate-400">{template.description}</p>
              </button>))) : (<p className="text-center text-slate-500 p-8">No hay plantillas de IA disponibles para la categoría "{category}".</p>)}
        </div>
      </div>
    </div>);
};
exports.SelectPromptTemplateModal = SelectPromptTemplateModal;
