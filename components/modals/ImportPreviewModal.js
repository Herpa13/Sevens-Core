...
exports.ImportPreviewModal = void 0;
...
const ErrorDetailItem = ({ error }) => (<CollapsibleSection_1.CollapsibleSection title={`Fila ${error.rowNumber}: ${error.message}`}>
...
const UpdateDetailItem = ({ update }) => (<CollapsibleSection_1.CollapsibleSection title={update.oldData.name || update.oldData.batchNumber || `ID: ${update.oldData.id}`}>
        <table className="min-w-full text-sm">
            ...
            <tbody>
                {update.changes.map((change, i) => (<tr key={i} className="border-b border-slate-700/50">
...
const ImportPreviewModal = ({ previewState, onClose, onConfirm }) => {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-lg shadow-xl w-full max-w-4xl border border-slate-700 max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto">
          {hasErrors && (<CollapsibleSection_1.CollapsibleSection title={`Se encontraron ${errors.length} Errores`} defaultOpen>
          ...
          <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            <h4 className="font-bold text-blue-300 flex items-center"><Icon_1.Icon name="info-circle" className="mr-2"/> Resumen de Cambios</h4>
          ...
          {updates.length > 0 && (<CollapsibleSection_1.CollapsibleSection title={`Detalle de Actualizaciones (${updates.length})`}>
                <div className="space-y-2">
                    {updates.map((upd, i) => (<UpdateDetailItem key={i} update={upd}/>))}
                </div>
            </CollapsibleSection_1.CollapsibleSection>)}
        ...
        <div className="p-4 border-t border-slate-700 flex justify-end space-x-2 flex-shrink-0">
          ...
          <button onClick={() => onConfirm(previewState)} disabled={!hasChanges} className="px-4 py-2 bg-green-500 text-white font-semibold rounded-md hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed">
            <Icon_1.Icon name="check" className="mr-2"/>
...
exports.ImportPreviewModal = ImportPreviewModal;
